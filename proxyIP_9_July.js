  ...
  if (subtitleLabel && subtitleContent) {
    if (pageType === 'file') {
      ...
      const ranges = subtitleContent
        .split(',')
        .map(r => `<span class="range-tag" data-copy="${r.trim()}">${r.trim()}</span>`)
        .join('<br>');
  ...
  async fetch(request, env, ctx) {
    ...
    if (path.toLowerCase().startsWith('/api/')) {
      ...
      if (path.toLowerCase() === '/api/check_file') {
        ...
        if (!targetUrl || !targetUrl.startsWith('http')) {
          return new Response(
            JSON.stringify({ success: false, error: 'Invalid or missing URL parameter' }),
            { status: 400, headers: { 'Content-Type': 'application/json' } },
          );
        }
        try {
          ...
          return new Response(JSON.stringify({ success: true, successful_ips: successfulIPs }), {
            headers: { 'Content-Type': 'application/json' },
          });
        } catch (e) {
